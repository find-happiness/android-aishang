...
public class SearchBox extends RelativeLayout {
  ...
  public SearchBox(final Context context, AttributeSet attrs, int defStyle) {
    ...
    search.addTextChangedListener(new TextWatcher() {
      ...
      @Override public void afterTextChanged(Editable s) {
        if (s.length() > 0) {
          ...
          mic.setImageDrawable(getContext().getResources().getDrawable(R.drawable.ic_action_mic));
    ...
    mSearchFilter = new SearchFilter() {
      @Override public boolean onFilter(SearchResult searchResult, String searchTerm) {
  ...
  /***
   * Hide the searchbox using the circle animation which centres upon the provided menu item. Can be
   * called regardless of result list length
   *
  ...
  /***
   * Set whether the menu button should be shown. Particularly useful for apps that adapt to screen
   * sizes
   *
  ...
  /***
   * Set whether to search without suggestions being available (default is true). Disable if your
   * app only works with provided options
   *
  ...
  /***
   * Set the image drawable of the drawer icon logo (do not set if you have not hidden the menu
   * icon)
   *
   * @param icon Icon
   */
  ...
  public void addSearchable(SearchResult searchable) {
    if (!searchables.contains(searchable)) searchables.add(searchable);
  }
  ...
  private void revealFrom(float x, float y, Activity a, SearchBox s) {
    FrameLayout layout =
        (FrameLayout) a.getWindow().getDecorView().findViewById(android.R.id.content);
  ...
  /***
   * Set to false to retain the logo from setDrawerLogo() instead of animating to the arrow during
   * searches.
   *
  ...
  public static class SearchAdapter extends ArrayAdapter<SearchResult> {
    ...
    @Override public View getView(int position, View convertView, ViewGroup parent) {
      ...
      if (convertView == null) {
        convertView =
            LayoutInflater.from(getContext()).inflate(R.layout.search_option, parent, false);

        if (mAnimate) {
          Animation anim = AnimationUtils.loadAnimation(getContext(), R.anim.anim_down);
